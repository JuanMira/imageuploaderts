...
const storage = multer.diskStorage({
  ...
  filename: function (req:Request<any, RequestQuery, RequestParams>, file, cb) {
    const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
    const filename:string = file.fieldname + "-" + uniqueSuffix + path.extname(file.originalname);
    cb(null, filename);
    const image = new Image({name:filename,path:dirname + `/uploads/${req.userId}`})
    image.save();
  },
});
